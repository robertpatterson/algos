...
//		$refList = $refList . "<br> NUM = $num  and count = $count<br>";
		addToRefList($num,$c);
...
function getCommonInfo()  // Send all the common info pages
{
	...
	global $jsonObj;
//	global $db;
	$sql = "SELECT alias, introtext, title
		FROM jos_content
		WHERE catid = 9";
//	echo "$sql\n";
	$res = mysqli_query($link,$sql);
...
function getAbstract()
{
	...
	*/
	...
	$url = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id=' . $jsonObj->pmid . '&retmode=XML&rettype=abstract';
